...
@Component({
  selector: "app-history-use",
  templateUrl: "./history-use.component.html",
  styleUrls: ["./history-use.component.scss"]
})
export class HistoryUseComponent implements OnInit {
  pageEvent: PageEvent;
  ...
  constructor(
    ...
    if (Number(this.activeRoute.snapshot.queryParams["company"]) != NaN) {
      this.companyId = Number(this.activeRoute.snapshot.queryParams["company"]);
      this.params.companyId = this.companyId;
    } else {
      this.companyId = -1;
      this.params.companyId = -1;
    }
    this.companyName = this.activeRoute.snapshot.queryParams["name"];
  ...
  onPay() {
    this.toggleRequest();
    this.appService
      .post(CompanyAPIs.PAY, this.toggleRequest, {
        month: this.month.value,
        year: this.year.value,
        companyId: this.companyId
      })
